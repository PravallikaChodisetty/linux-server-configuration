...
Hosted site Url http://13.127.162.197.xip.io/
...
4. Creating grader User: 
   
   sudo adduser grader : This will add new user

5. sudo nano /etc/sudoers

6. Below the Root user append the following line
...
Note: Before Logging using ssh add custom TCP port 2200 under lightsail firewall in networking tab in lightsail instance console
...
After that go to lightsail account

In networking change firewall setting
...
·  Add the below three links as google authorized links
...
·  Download the corresponding JSON file, open and copy the contents.
